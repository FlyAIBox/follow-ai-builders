...
> The digest **core layer** is these **34 curated sources** (26 X + 6 podcasts + 2 blogs). See [`config/default-sources.json`](config/default-sources.json). The BestBlogs extended layer runs in parallel and is included when content is available (below).
...
The extended layer is wired into the digest via `feed-bestblogs.json` and runs **in parallel** with the curated tier — daily digests include **both** when content is available. Curated sources emphasize depth (official X API, full podcast transcripts); BestBlogs emphasizes breadth (400 RSS feeds, mostly titles and RSS summaries, no transcripts). The OPML / JSON catalog is also useful for discovery and expansion.
...
2. **Central feed generation** — `scripts/generate-feed.js` and
   `scripts/generate-bestblogs-feed.js` run in the maintainer environment or GitHub
   Actions. The curated scripts fetch X posts, podcast RSS/transcripts, and official
   blog articles; BestBlogs aggregates 400+ RSS feeds. Both deduplicate with
   `state-feed.json` and write `feed-x.json`, `feed-podcasts.json`,
   `feed-blogs.json`, and `feed-bestblogs.json`.
...
This repository is a **secondary development** (fork & extension) of the upstream project [**follow-builders**](https://github.com/zarazhangrui/follow-builders).

| Item | Notes |
|------|-------|
| **Code & skill architecture** | Based on [zarazhangrui/follow-builders](https://github.com/zarazhangrui/follow-builders), extended here with Cursor/Codex support and the BestBlogs catalog |
| **Default content sources** | The curated tier (26 X builders, 6 podcasts, 2 official blogs) and the central `feed-*.json` pipeline originate from the upstream project |
| **License** | MIT, consistent with upstream |

Thanks to [Zara Zhang](https://x.com/zarazhangrui) and the upstream community for the original design and curated sources.

To suggest changes to the default source list, please open an issue on the [upstream repository](https://github.com/zarazhangrui/follow-builders/issues).
